...
void CommandCalc::Track_run() {
  ...
  case Start_to_1st_Corner:
    ...
    tail_mode_lflag = false;
  	...
  	//20170914 h.taga ガレージ格納後Stopさせるためのフラグ
  	check_garage = false;
  ...
  case Go_LUG:
    ...
    if(mOdo >= ref_odo){
      LookUpGateRunner(mLinevalue_LUG+40, mOdo, mYawangle,mLinevalue+40);
    }
  ...
  case Return_to_Line_Garage:
    ...
    forward =  speedcal;
  	...
  	//20170914 h.taga ガレージ格納後Stopさせる
  	check_garage = true;
...
//20170914 h.taga ガレージ停車時に車体を完全に停止させるフラグを追加(check_garage)
